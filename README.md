...
To run the OpenTok Moderation Sample App, run the following commands:
...
- **[viewer.js](./public/js/viewer.js)**: Viewers can only view the session.
...
connect to the session and subscribe to the session. Each viewer uses the OpenTok
[Signaling API](https://www.tokbox.com/developer/guides/signaling/js/) to receive the
signals sent in the session.
